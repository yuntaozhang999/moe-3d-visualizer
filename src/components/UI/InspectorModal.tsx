...
export const INSPECTION_DATA: {
  ...
  node_w_q: {
    ...
    category: "Attention Branch / Parameters",
    formula: "W_Q \\in \\mathbb{R}^{6144 \\times 6144}",
    realShape: "[6144, 6144]",
    visualShape: "[16, 16]",
    codeSnippet: "self.w_q = _init_weight((6144, 6144))",
    rationale: "Projects hidden states into 48 Query heads (dim 128).",
  },
  node_w_k: {
    title: "Key Weight Matrix (W_K)",
    category: "Attention Branch / Parameters",
    formula: "W_K \\in \\mathbb{R}^{6144 \\times (H_{kv} \\times 128)}",
    realShape: "Local: [6144, 1536] / Global: [6144, 768]",
    visualShape: "Local: [8, 16] / Global: [4, 16]",
    codeSnippet: "self.w_k = _init_weight((6144, kv_heads * 128))",
    rationale: "GQA compression. Projects hidden states into Key heads. Ratio is 4:1 (12h) for Local layers, and 8:1 (6h) for Global layers.",
  },
  node_w_v: {
    ...
    rationale: "GQA compression. Projects hidden states into Value heads. Ratio is 4:1 (12h) for Local layers, and 8:1 (6h) for Global layers.",
  },
  op_q_proj: {
    title: "Query Projection (@)",
    category: "Attention Branch / Operator",
    formula: "Q = X @ W_Q",
    realShape: "[1, 4096, 6144] → [1, 4096, 48, 128]",
    visualShape: "Operator",
    codeSnippet: "q_flat = jnp.einsum('bsh,hd->bsd', x, self.w_q)",
    rationale: "Computes the query projections for all 48 heads.",
  },
  op_k_proj: {
    title: "Key Projection (@)",
    category: "Attention Branch / Operator",
    formula: "K = X @ W_K",
    realShape: "[1, 4096, 6144] → [1, 4096, H_kv, 128]",
    visualShape: "Operator",
    codeSnippet: "k_flat = jnp.einsum('bsh,hd->bsd', x, self.w_k)",
    rationale: "Computes the key projections for KV heads. GQA reduces memory footprint.",
  },
  op_v_proj: {
    ...
    realShape: "[1, 4096, 6144] → [1, 4096, H_kv, 128]",
    visualShape: "Operator",
    codeSnippet: "v_flat = jnp.einsum('bsh,hd->bsd', x, self.w_v)",
    rationale: "Computes the value projections for KV heads. GQA reduces memory footprint.",
  },
  node_q: {
