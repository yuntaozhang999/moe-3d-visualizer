...
async function run() {
  ...
  // MoE Focus Z-Projection
  console.log('Setting Focus Z-Projection view...');
  await page.evaluate(() => {
    if (window.__threeCamera && window.__threeControls) {
      window.__threeCamera.position.set(24.0, 10.0, 14.0);
      window.__threeControls.target.set(24.0, 2.0, -2.5);
  ...
  await page.evaluate(() => {
    if (window.__threeCamera && window.__threeControls) {
      window.__threeCamera.position.set(26.0, 25.0, -0.01);
      window.__threeControls.target.set(26.0, 0, 0);
      window.__threeControls.update();
    }
  });
  await new Promise(resolve => setTimeout(resolve, 1000));
